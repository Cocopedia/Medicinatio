...
class search_class extends AWS_MODEL
{
	...
	public function search_users($q, $page, $limit = 20)
	{
		...
		return $this->query_all('SELECT uid, last_login FROM ' . get_table('users') . ' WHERE ' . implode(' OR ', $where).' order by user_name asc', calc_page_limit($page, $limit));
	}
	...
	public function prase_result_info($result_info,$q)
	{
		...
		else if ($result_info['question_id'])
		{
			...
			$info=mb_chunk_split($q,$result_info['question_detail'],50);
			$detail = array(
		...
		else if ($result_info['id'])
		{
			...
			$info=mb_chunk_split($q[0],$result_info['message'],50);
			$detail = array(
