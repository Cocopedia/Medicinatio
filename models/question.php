...
class question_class extends AWS_MODEL
{
    ...
    public function get_by_like($q, $page = 1, $limit = 1, $is_recommend = false)
    {
        ...
        $offset = ($page-1) * $limit;
        $sql = "select * from ".$this->get_table('question')." where question_content regexp'".$this->quote(implode('|', $q))."' or question_detail regexp'".$this->quote(implode('|', $q))."' order by add_time desc limit ".calc_page_limit($page,$limit);
